...
server <- function(input, output) {
    ...
    fR <- reactive(forward.jacobian(func(), X()))
    fT <- reactive(system.time(forward.jacobian(func(), X())))
    output$fT <- renderText(fT()[3])
    # output$fR <- renderText(fR())

    rR <- reactive(reverse.jacobian(func(), X()))
    rT <- reactive(system.time(reverse.jacobian(func(), X())))
    tp <- reactive(reverse.jacobian.tape(func(), X()))
    tR <- reactive(reverse.jacobian(tp(), X()))
    tT <- reactive(system.time(reverse.jacobian(tp(), X())))
