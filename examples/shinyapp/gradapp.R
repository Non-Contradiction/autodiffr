...
server <- function(input, output) {
    ...
    fR <- reactive(forward.grad(func(), X()))
    fT <- reactive(system.time(forward.grad(func(), X())))
    output$fT <- renderText(fT()[3])
    # output$fR <- renderText(fR())

    rR <- reactive(reverse.grad(func(), X()))
    rT <- reactive(system.time(reverse.grad(func(), X())))
    tp <- reactive(reverse.grad.tape(func(), X()))
    tR <- reactive(reverse.grad(tp(), X()))
    tT <- reactive(system.time(reverse.grad(tp(), X())))
