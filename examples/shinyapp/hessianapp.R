...
server <- function(input, output) {
    ...
    fR <- reactive(forward_hessian(func(), X()))
    fT <- reactive(system.time(forward_hessian(func(), X())))
    output$fT <- renderText(fT()[3])
    # output$fR <- renderText(fR())

    rR <- reactive(reverse_hessian(func(), X()))
    rT <- reactive(system.time(reverse_hessian(func(), X())))
    tp <- reactive(reverse_hessian_tape(func(), X()))
    tR <- reactive(reverse_hessian(tp(), X()))
    tT <- reactive(system.time(reverse_hessian(tp(), X())))
